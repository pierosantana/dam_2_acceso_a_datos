...
import modelo.persistencia.interfaz.IDaoCoche;
...
public class DaoImplement implements IDaoCoche {
	 ...
	 * @return 1 en caso de que fue agregado correctamente, 0 en caso contrario.
	 */
	@Override
	...
	public List<Coche> getAllCoches() {
		...
		try (Connection con = DriverManager.getConnection(cadenaConexion, user, pass);) {
			...
			while (rs.next()) {
				...
				coche.setNumeroPasajeros(rs.getInt("CANTPASAJEROS"));
	 ...
	 * @return 1 en caso de que fue actualizado correctamente, 0 en caso contrario.
	 */
	@Override
	public int updateCoche(Coche coche) {
		int rowsAffected = 0;
		try (Connection con = DriverManager.getConnection(cadenaConexion, user, pass);) {
			String sql = "UPDATE COCHES SET MARCA=?, MODELO=?, MOTOR=?, KILOMETROS=?, CANTPASAJEROS=? WHERE ID=?";
			PreparedStatement ps = con.prepareStatement(sql);
			ps.setString(1, coche.getMarca());
			ps.setString(2, coche.getModelo());
			ps.setString(3, coche.getMotor().toString());
			ps.setDouble(4, coche.getKilometros());
			ps.setInt(5, coche.getNumeroPasajeros());
			ps.setInt(6, coche.getId());
	 ...
	 * @return Objeto de tipo Coche, si existe en la base de datos, null en caso
	 *         contrario.
	 */
	@Override
	...
	/**
	 * Retorna todos los coches que tengan la misma Marca guardados en la base de
	 * datos.
	 *
	 * @param marca de los coches a buscar.
	 * @return Lista de Coches, si existe almenos 1 en la base de datos, null en
	 *         caso contrario.
	 */
	@Override
